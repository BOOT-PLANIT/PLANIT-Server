...
public class FirebaseAccountService {
  ...
  public UserDetails ensureAndLoad(FirebaseToken token) {
    ...
    var found = mapper.findByUidIncludingDeleted(uid);
    ...
    if (found.isPresent()) {
      ...
      var userAccount =
          UserAccount.builder().uid(uid).email(email).displayName(displayName).photoUrl(photoUrl)
    ...
    return User.withUsername(uid).password("N/A").authorities(authorities).build();
  }

  /** userId 조회 */
  public Long findUserIdByUid(String uid) {
    return mapper.findByUid(uid).map(UserAccount::getId)
