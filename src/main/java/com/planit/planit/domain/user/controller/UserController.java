...
public class UserController {
  ...
  public ResponseEntity<ApiResponse<MeResponseDTO>> me(Authentication auth) {
    ...
    MeResponseDTO meResponseDTO = MeResponseDTO.builder().id(user.getId()).uid(user.getUid())
        .email(user.getEmail()).displayName(user.getDisplayName()).photoUrl(user.getPhotoUrl())
        .userLevel(user.getUserLevel()).provider(user.getProvider())
        .emailVerified(user.isEmailVerified()).createdAt(user.getCreatedAt())
        .lastLoginAt(user.getLastLoginAt()).recentBootcampId(recentBootcampId).build();
  ...
  @Operation(summary = "회원 탈퇴", security = {@SecurityRequirement(name = "BearerAuth")},
      responses = {
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "204",
              description = "탈퇴 성공", content = @Content),
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401",
              description = "인증이 필요합니다.", content = @Content(mediaType = "application/json")),
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404",
  ...
  @Operation(summary = "FCM 토큰 저장/갱신", security = {@SecurityRequirement(name = "BearerAuth")},
      responses = {
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200",
              description = "토큰 저장 성공"),
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400",
              description = "fcmToken 누락 또는 잘못된 값"),
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401",
              description = "인증 필요"),
          @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404",
              description = "사용자 없음")})
  ...
  public ResponseEntity<Void> saveFcmToken(Authentication auth,
